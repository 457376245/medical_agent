...
def test_disease_profile_delete_endpoint_contract_present() -> None:
    openapi = read_text("specs/001-medical-agent-mvp/contracts/openapi.yaml")
    assert_contains_all(
        openapi,
        [
            "/disease-profiles/{diseaseProfileId}:",
            "operationId: deleteDiseaseProfile",
            "name: onlyIfEmpty",
            "'409':",
            "ApiResponseDiseaseProfileDelete",
        ],
    )
