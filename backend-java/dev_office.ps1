...
# Set console to UTF-8 for Chinese character support on Windows
...
function Resolve-JavaHomePath {
    ...
    if (-not [string]::IsNullOrWhiteSpace($ConfiguredJavaHome)) {
        $candidatePaths += $ConfiguredJavaHome
    }
    $candidatePaths += @(
        "D:\JDK21",
        "D:\jdk-21.0.7"
    ...
    throw "Configured JavaHome '$ConfiguredJavaHome' was not found, and no fallback Java 21 installation was detected."
}

$JavaHome = Resolve-JavaHomePath -ConfiguredJavaHome $JavaHome
...
if ($EnvFile) {
    ...
    Write-Host "Env file: <none> (using Spring profile defaults)"
}
