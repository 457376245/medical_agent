...
- Add disease-centric timeline query endpoints and data-rights request endpoints (export/delete) under `/api/v1`.
...
- **API compatibility rollback**: Keep `/api/v1` backward compatible; disable new features via feature flag without removing existing endpoints.
...
- All external APIs remain under `/api/v1`; new capabilities are additive and backward compatible.
...
- **V. Versioned Contracts & Measurable Delivery**: PASS - `/api/v1`, MQ topic `.v1`, structured schema version, measurable latency targets.
