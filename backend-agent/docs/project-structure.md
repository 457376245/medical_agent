...
| Agent 编排 | 项目内轻量 `AgentRuntime` |
| LLM 接入 | OpenAI 官方 Python SDK + OpenAI 兼容 Chat Completions |
...
app/
  ...
  agent/
    runtime.py            轻量 Agent 运行循环
    messages.py           Agent 消息和工具调用模型
    events.py             Runtime 对外流式事件
    state.py              每个 thread_id 的运行态
    tool_runner.py        工具执行和重复失败短路
    prompting.py          LLM 输入 prompt 组装和消息裁剪
    context.py            疾病档案上下文签名、解析、系统消息构造
  tools/
    registry.py           工具注册表和 OpenAI tool schema
  ...
  memory/
    store.py              SQLite 会话索引、turn、trace 和 Agent 运行态存储
...
2. `AgentRuntime` 从 `SqliteMemoryStore` 读取运行态。
3. 如 metadata 中的疾病档案签名变化，runtime 先调用 `fetch_disease_profile_context` 并缓存上下文。
4. `prompting.py` 组装系统提示词、附件提示、疾病档案上下文、场景提示和工具失败提示。
5. Runtime 通过 OpenAI SDK 流式调用 chat completions。
6. 如果模型返回工具调用，`tool_runner.py` 执行工具并把结果追加回消息历史；超过 `MAX_TOOL_ROUNDS` 后停止。
7. `chat.py` 将 runtime 事件转换为 SSE：`session`、`token`、`tool_call`、`tool_result`、`done`、`error`。
8. 结束时保存 turn、trace、session 摘要和 runtime state。
...
| `agent_runtime_states` | 每个 `thread_id` 的消息历史和当前疾病档案上下文缓存 |

`MEMORY_DB_PATH` 默认指向 `data/memory.db`。
